	...
	return {
		...
		characterPack:{
			mode_guozhan:{
				...
				//gz_caozhi:['male','wei',3,['jiushi','luoying']],
				...
				gz_caoren:['male','wei',4,['xinjushou','xinjiewei']],
				...
				gz_caopi:['male','wei',3,['xingshang','fangzhu'],['gzskin']],
				gz_yuejin:['male','wei',4,['xiaoguo'],['gzskin']],
				...
				//useCardToPlayeredlongdansha->chongzhen,longdanshan->oldshushen
				gz_zhaoyun:['male','shu',4,['longdan','new_yajiao']],
				//notsuitbutcolor
				...
				//zaiqi+1
				gz_menghuo:['male','shu',4,['huoshou','zaiqixx']],
				...
				gz_lvmeng:['male','wu',4,['keji','new_mouduan']],
				...
				//gz_re_yuanshao:['male','qun',4,['luanji'],['gzskin']],
				...
				//gz_caohong:['male','wei',4,['yuanhu','heyi'],['gzskin']],
				...
				//gzqianxi
				gz_madai:['male','shu',4,['mashu2','gzqianxi'],['gzskin']],
				...
				//gz_yujin:['male','wei',4,['yizhong'],['gzskin']],
				//gz_shen_caocao:['male','wei',3,['guixin']],
				//gz_wangping:['male','shu',4,['jianglue'],['gzskin']],
				gz_fazheng:['male','shu',3,['gzxuanhuo','gzenyuan'],['gzskin']],
				//gz_wuguotai:['female','wu',3,['buyi','ganlu'],['gzskin']],
				//gz_lukang:['male','wu',3,['keshou','zhuwei'],['gzskin']],
				//gz_yuanshu:['male','qun',4,['yongsi'],['gzskin']],
		...
		skill:{
			...
			xindiaodu:{
				...
				filter:function (event,player){
					return game.hasPlayer(function(current){
						return current.isFriendOf(player)&&current!=player&&current.countGainableCards(player,'e')>0;
					});
				},
				direct:true,
				content:function (){
				...
				subSkill:{
					others:{
						...
						content:function(){
							...
							'step 1'
							var list=['wusheng','new_paoxiao','longdan','new_tieji','liegong','xinkuanggu'];
							for(var i=0;i<list.length;i++){
								if(game.hasPlayer(function(current){
							...
							if(result.control=='longdan')
								player.addTempSkill('fz_new_longdan');
							else
			...
			fz_new_longdan:{
				audio:true,
				group:["fz_new_longdan_sha","fz_new_longdan_shan","fz_new_longdan_draw","fz_new_longdan_discard"],
				subSkill:{					
					/*draw:{
						trigger:{
					...
					draw:{
						...
						filter:function(event,player){
							//if(!get.zhu(player,'shouyue')) return false;
							return event.skill=='fz_new_longdan_shan';//event.skill=='longdan_shan';
						},
						prompt:'可以令一名其他角色摸一张牌',
						content:function(){
							'step 0'
					...
					discard:{
						...
						filter:function(event,player){
							//if(!get.zhu(player,'shouyue')) return false;
							return event.skill=='fz_new_longdan_sha';//event.skill=='longdan_sha';
						},
						prompt:'可以弃置一名其他角色一张手牌',
						content:function(){
							'step 0'
							player.chooseTarget(function(card,player,target){
							...
							'step 1'
							if(result.bool){
								player.discardPlayerCard(result.targets[0],'h');
							}
						}						
					},
					sha:{
			...
			"new_longdan":{
				...
				subSkill:{
					shanafter:{
						...
						filter:function (event,player){
							return event.skill=='fz_new_longdan_shan'&&game.hasPlayer(function(current){
					...
					shamiss:{
						...
						direct:true,
						filter:function (event,player){
							return event.skill=='new_longdan_sha'&&event.card=='sha';
						},
						content:function (){
			...
			"new_chuli":{
				...
				content:function (){
					...
					"step 1"
					if(result.bool){
						if(get.suit(result.cards[0])=='spade'&&(!target.hasSkill('hongyan'))) event.getParent().draw.push(target);
					}
				},
			...
			"new_tieji":{
				...
				content:function (){
					...
					"step 3"
					var suit=get.color(result.card);
					var target=trigger.target;
					var num=target.countCards('h','shan');
					target.chooseToDiscard('请弃置一张'+get.translation(suit)+'牌，否则不能使用闪抵消此杀','he',{color:suit}).set('ai',function(card){
				...
				chooseButton:{
					dialog:function(){
						var list=[
							'taoyuan','wugu','juedou','huogong','jiedao','tiesuo','guohe','shunshou','wuzhong','wanjian','nanman',
							'yuanjiao','zhibi','yiyi',
							//'xietianzi','shuiyanqijunx','lulitongxin','lianjunshengyan','chiling','diaohulishan','huoshaolianying',
						];
						for(var i=0;i<list.length;i++){
							list[i]=['锦囊','',list[i]];
						}
			...
			gzxiongsuan:{
				...
				content:function(){
					...
					'step 1'
					if(!target.sameIdentityAs(player)) return;
					player.draw(3);
					var list=[];
				...
				content:function(){
					...
					if(!target.countCards('h')){
						...
						player.chooseControl().set('choiceList',[
							'观看'+get.translation(target)+'的手牌并可以弃置其中的一张红桃牌',
		...
		jianglue:'将略',
			...
			gz_fazheng:'法正',
			gzxuanhuo:'眩惑',
			gzxuanhuo_info:'若此武将牌明置，与你势力相同的其他角色的出牌阶段限一次，该角色可以选择两张牌，交给你第一张弃置第二张，然后获得以下一项场上没有的技能直到回合结束：〖武圣〗、〖咆哮〗、〖龙胆〗、〖铁骑〗、〖烈弓〗、〖狂骨〗。',
			gzenyuan:'恩怨',
			...
			"new_jushou":"据守",
			"new_jushou_info":"结束阶段，你可以摸X张牌（X为当前体力值），然后弃置一张手牌。若以此法弃置的牌为装备牌，则改为使用此牌。若X大于2，则你将武将牌叠置。",
			"new_duanliang":"断粮",
			...
			"new_fenji":"奋激",
			"new_fenji_info":"一名角色的结束阶段开始时，若其没有手牌，你可以令其摸三张牌，然后你失去1点体力并弃一张牌。",
			"new_luanji":"乱击",
			...
			"new_kongcheng":"空城",
			"new_kongcheng_info":"锁定技，若你没有手牌，当你成为【杀】或【决斗】的目标时，取消之。",
			"new_keji":"克己",
			"new_keji_info":"锁定技，若你没有在出牌阶段内使用过颜色不同的牌，则你本回合的手牌上限+4。",
			"keji_add":"克己",
			"keji_add_info":"",
			"new_mouduan":"谋断",
			"new_mouduan_info":"弃牌阶段开始时，若你的手牌数大于体力值，你可以移动场上的一张牌。",
			...
			"fz_new_longdan":"龙胆",
			"fz_new_longdan_info":"你可以将【杀】当做【闪】使用或打出，并可以令一名其他角色摸一张牌，或将【闪】当做【杀】使用或打出，并可以弃置一名其他角色一张手牌。",
			"new_paoxiao":"咆哮",
			...
			"kurou_effect_info":"",
			"new_chuli":"除疠",
			"new_chuli_info":"出牌阶段限一次，若你有牌，你可以选择至多三名势力各不相同的其他角色，你弃置你和这些角色的各一张牌。然后所有以此法弃置过黑桃牌的角色各摸一张牌。",
			"baka_hunshang":"魂殇",
			...
			"fengyin_vice_info":"",
			"new_tieji":"铁骑",
			"new_tieji_info":"当你使用【杀】指定目标后，你可以令其一张明置的武将牌上的非锁定技于本回合内失效，然后你进行判定，除非该角色弃置与结果颜色相同的一张牌，否则其不能使用【闪】响应此【杀】。",
			hmkyuanyu:"远域",
			...
			qianhuan:'千幻',
			qianhuan_bg:'幻',
			qianhuan_info:'当与你势力相同的一名角色受到伤害后，你可以将一张与你武将牌上花色均不同的牌置于你的武将牌上。当一名与你势力相同的角色成为基本牌或锦囊牌使用时的唯一目标时，你可以移去一张“千幻”牌，取消之。',
			gzzhiman:'制蛮',
			gzzhiman_info:'当你对其他角色造成伤害时，你可以防止此伤害。若如此做，你获得其装备区或判定区里的一张牌。然后若该角色与你势力相同，该角色可以变更副将。',
			
			gzdiancai:'典财',
			gzdiancai_info:'其他角色的出牌阶段结束时，若你于此阶段失去了x张或更多的牌，则你可以将手牌摸至体力上限。（x为你的体力值）。',
			xuanlve:'旋略',
			...
			gzqice:'奇策',
			gzqice_backup:'奇策',
			gzqice_info:'出牌阶段限一次，你可以将所有手牌当做任意一张普通锦囊牌使用（此牌的目标数不能超过你的手牌数）。',
			gzyuejian:'约俭',
			gzyuejian_info:'锁定技，与你势力相同角色的弃牌阶段开始时，若其本回合未使用牌指定过其他势力的角色为目标，则该角色本回合手牌上限+X（X为其已损失的体力值）。',
			gzxiongsuan:'凶算',
			gzxiongsuan_info:'限定技，出牌阶段，你可以弃置一张手牌并选择一名角色，对其造成1点伤害。若其与你势力相同，你摸三张牌，然后若该角色有已发动的限定技，则你选择其中一个限定技，此回合结束后视为该限定技未发动过。',
			gzhuashen:'化身',
			...
			gzshangyi:'攻心',
			gzshangyi_info:'出牌阶段限一次，你可以令一名其他角色观看你的手牌。若如此做，你选择一项：1.观看其手牌并可以弃置其中的一张红桃牌；2.观看其所有暗置的武将牌。',
			niaoxiang:'鸟翔',
			niaoxiang_sha:'鸟翔',
			niaoxiang_info:'阵法技，在同一个围攻关系中，若你是围攻角色，则你或另一名围攻角色使用【杀】指定被围攻角色为目标后，该角色需依次使用两张【闪】才能抵消。',
			yicheng:'疑城',
			yicheng_info:'当与你势力相同的一名角色成为【杀】的目标后，你可以令该角色摸两张牌，弃置一张牌，然后你弃置一张牌。',
			yizhi:'遗志',
			...
			ziliang:'资粮',
			ziliang_info:'副将技，当与你势力相同的一名角色受到伤害后，你可以将一张“田”交给该角色，然后你摸一张牌。',
			gzjixi:'急袭',
