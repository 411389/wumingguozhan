	...
	return {
		...
		skill:{
			...
			jianxiong:{
				...
				content:function(){
					"step 0"
					player.chooseControl('获得造成伤害的牌','摸一张牌');
					"step 1"
					if(result.control=='获得造成伤害的牌')
			...
			guicai:{
				...
				direct:true,
				filter:function(event,player){
					return player.countCards(get.mode()=='guozhan'?'h':'h')>0;
				},
				content:function(){
					"step 0"
					player.chooseCard(get.translation(trigger.player)+'的'+(trigger.judgestr||'')+'判定为'+
					get.translation(trigger.player.judging[0])+'，'+get.prompt('guicai'),get.mode()=='guozhan'?'h':'h',function(card){
			...
			longdan:{
				...
				group:['longdan_sha','longdan_shan','longdan_draw','longdan_discard'],
				subSkill:{
					draw:{
						...
						filter:function(event,player){
							//if(!get.zhu(player,'shouyue')) return false;
							return event.skill=='longdan_shan';
						},
			...
			lijian:{
				...
				multitarget:true,
				content:function(){
					targets[1].useCard({name:'juedou',isCard:true},targets[0],'noai').animate=false;
					game.delay(0.5);
				},
				ai:{
					order:8,
		...
		translate:{
			...
			jianxiong:'奸雄',
			jianxiong_info:'当你受到伤害后，你可以获得对你造成伤害的牌，或另摸一张牌。',
			...
			guicai_info_guozhan:'一名角色的判定牌生效前，你可以打出一张手牌代替之。',
			...
			longdan_info:'你可以将【杀】当做【闪】使用或打出，并可以令一名其他角色摸一张牌，或将【闪】当做【杀】使用或打出，并可以弃置一名其他角色一张手牌。',
