	...
	return {
		...
		skill:{
			...
			buyi:{
				...
				filter:function(event,player){
					return event.player.hp<=0&&event.player.countCards('h')>0&&event.player.isFriendOf(player);
				},
				direct:true,
				content:function(){
					"step 0"
		...
		translate:{
			...
			xinjingce_info:'结束阶段，若你本回合使用牌的类别数量大于或等于你的当前体力值，你可以摸两张牌。',
			...
			jiushi_info:'当你需要使用一张【酒】时，若你武将牌正面朝上，则你可以将武将牌翻面并视为使用了一张【酒】。',
			...
			yizhong_info:'锁定技，当你的防具栏为空时，黑色的杀指定你为目标，取消之',
			...
			buyi_info:'当有同势力角色进入濒死状态时，你可以展示该角色的一张手牌：若此牌不为基本牌，则该角色弃置此牌并回复1点体力。',
