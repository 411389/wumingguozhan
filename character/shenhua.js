	...
	return {
		...
		skill:{
			...
			xinjiewei:{
				...
				viewAs:{name:'wuxie'},				
				filter:function(event,player){
					return player.countCards('e')>0&&player.classList.contains('turnedover');
				},
			...
			tiaoxin:{
				...
				content:function(){
					...
					"step 1"
					if(result.bool!=true&&target.countCards('he')>0){
		...
		translate:{
			...
			xinjiewei_info:'当你的武将牌背面朝上时，可以将装备区里的牌当【无懈可击】使用。',
			...
			tiaoxin_info:'出牌阶段限一次，你可以指定一名攻击范围内包含你的角色，若该角色主将明置，需对你使用一张【杀】，否则你弃置其一张牌。',
			...
			weimu:'帷幕',
			jiezi:'截辎',
			jiezi_info:'锁定技，一名角色跳过摸牌阶段后，你摸一张牌。',
			...
			luanwu_info:'限定技，出牌阶段，你可令除你外的所有角色依次对与其距离1以内的另一名角色使用一张【杀】，否则失去1点体力。',
