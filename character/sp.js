	...
	return {
		...
		skill:{
			...
			chuanxin:{
				...
				filter:function(event,player){
					...
					if(event.card&&(event.card.name=='sha'||event.card.name=='juedou')&&event.getParent().name==event.card.name){
						if(get.mode()=='guozhan'){
							return (event.player.identity!='qun'||!player.wontYe())&&
			...
			zhendu:{
				...
				trigger:{global:'phaseUseBegin'},
				filter:function(event,player){
					return (get.mode()!='guozhan'||event.player!=player)&&player.countCards('h')>0&&event.player.hp>0;
				},
				direct:true,
				content:function(){
					"step 0"
			...
			qiluan:{
				...
				filter:function(event,player){
					return game.hasPlayer2(function(current){						
						return current.getStat('kill')>0&&player.isFriendOf(current)
					});
				},
				prompt:function(event,player){
					var num=game.countPlayer2(function(current){
						return (current.getStat('kill')||0)*(player.sameIdentityAs(current)?3:0);
					});
			...
			shushen:{
				...
				content:function(){
					...
					event.count=trigger.num;
					"step 1"
					player.chooseTarget(get.prompt('shushen'),'令一名其他角色选择摸一张牌',function(card,player,target){
						return target!=player;
					}).set('ai',function(target){
						return get.attitude(_status.event.player,target);
					});
		...
		translate:{
			...
			chuanxin_info_guozhan:'当你使用【杀】或【决斗】对目标角色造成伤害时，若目标角色明置与你不同势力且有副将，你可以防止此伤害。若如此做，该角色选择一项：1.弃置装备区里的所有牌，若如此做，其失去1点体力；2.移除副将。',
			hengjiang:'横江',
			hengjiang2:'横江',
			...
			shuangren_info_guozhan:'出牌阶段开始时，你可以与一名角色拼点。若你赢，你视为对其或与其势力相同的另一名角色使用一张【杀】（不计入出牌阶段的次数限制）；若你没赢，你于本回合出牌阶段内使用的牌不能指定其他角色为目标。',
			xiashu:'下书',
			...
			qiluan_info:'一名角色的回合结束时，与你同势力角色每击杀一名角色，你可以摸三张牌。',
			...
			xiongyi_info_guozhan:'限定技，出牌阶段，你可以令与你势力相同的所有角色各摸三张牌，然后若你的势力人数是全场最少的(或之一)，则你回复1点体力。',
